...
def Arnoldi_Tikhonov(A, b, L = 'Identity', projection_dim=3, iter=50, dp_stop = 0, param_choice = 'manual', automatic_param = 'gcv', reg_param = 1, delta = 0, **kwargs):
    if A.shape[0] == A.shape[1]:
        if L == 'Identity': ## This is Arnoldi Tikhonov in standard form
            b_vec = b.reshape((-1,1))
            ...
            b_vec = b.reshape((-1,1))
            (V,H) = arnoldi(A, projection_dim, b_vec, **kwargs)
            # L = np.identity(H.shape[1], dtype='float32')
            UU = V[:, 0:-1]
            HH = H[0:-1, :]
            bhat = UU.T.dot(b_vec)
            if param_choice == 'manual':
                ...
                if automatic_param == 'gcv':
                    ...
                    L = np.identity(HH.shape[1], dtype='float32')
                    reg_param = discrepancy_principle(HH, bhat, L, 1.01, delta, **kwargs)['x'] # find ideal lambdas by crossvalidation
                y = np.linalg.solve(HH.T*HH + reg_param[0]*L, bhat)
                x_AT = UU.dot(y)
    else:
